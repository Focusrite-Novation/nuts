...
function toSemver(tag) {
    ...
    if (isNaN(prerelease)) {
      if (parts[3]) {
        var results = parts[3].match(/^(alpha|beta|dev)(\d+)/)
        return version + '-' + results[1] + '.' + results[2];        
      } else { // no prerelease
        return version;
      }
    } else {
...
function parseRELEASES(content) {
    return _.chain(stripBom(content))
        ...
        .map(function(line) {
            ...
            console.log("FN parts", filenameParts);

            var version = _.chain(filenameParts)
                .filter(function(x) {
                    return /^(alpha|beta|dev)?\d+$/.exec(x);
                })
                .reverse()
                .value()
                .join('.');
            // var version = versionArr.join(".");
            // // prerelease! channel!
            //
            // if (versionArr.length === 4 && versionArr[versionArr.length - 1].match(/(alpha|beta|dev)/)) {
            //
            //   version = versionArr.slice(0,-1).join('.') + '-' +
            // }

            console.log("VERSION AFTER RELEASES", version);
            return {
                sha: parts[1],
                filename: filename,
                size: Number(parts[3]),
